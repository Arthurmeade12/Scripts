...
LATEST_INST=0
JAR='https://maven.fabricmc.net/net/fabricmc/fabric-installer/0.8.0/fabric-installer-0.8.0.jar'
out "Welcome to Arthur's Fabric installer!" 'This script only installs for 1.17.1.' 'Downloading installer ...'

curl $JAR -o ~/Downloads/fabric-installer.jar
### Env choice
if [[ -z $ENV ]]
then
...
out "Would you like a specific loader version ? If you don't know what this is, leave it blank. Answer list to see possible versions." "Possible answers: $(tput setaf 2)(wanted version), latest, (blank), list"
while a=$((a ++)); do
	prompt "What version?"
	read -t 30 VER
	...
	list)
		...
		a=$((a --)) # Not an invalid answer, but we want to restart loop
		continue
		;;
	*)
		:
...
echo
java -jar ~/Downloads/fabric.jar $ARGS
STATUS=$?
case $STATUS in
0)
