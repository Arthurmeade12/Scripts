...
YELLOW=`tput setaf 3`
# Readonly vars
for X in {BLUE,BOLD,RESET,RED,WHITE,SUDO}; do
	readonly -- $X
done
### Functions
out(){
...
qq(){
	echo -en "${BLUE}${BOLD}==>${RESET} ${WHITE}${X} ${YELLOW}"
}
# Readonly funcs
...
### Introduction
out "Welcome to Arthur's Desktop changer!" "This script lets you change your desktop image to whatever you like, with a few exceptions."
out "$YELLOW${UNDERLINED}You cannot have different images for each desktop, they must all be the same."
out "$YELLOW${UNDERLINED}The image must be in jpeg (.jpg or .jpeg) or png (.png) format. More formats may be added in the future."
qq "Would you like to continue? (y/n)"
while a=$((a++)); do
	read -n 2 -t 30 CONSENT
	test $? -gt 128 && echo && abort "30 seconds of inactivity. \nAborting ..."
	echo
	case $CONSENT in
	y)
		out "Great! Setting up environment ..."
		;;
	n)
